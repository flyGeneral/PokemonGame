...
check("初始招式", [x.name for x in m.moves] == ["抓", "叫声", "火花"])
foe = Mon("麻雀雏", 4)
...
msgs, evo = Mon("芽叶兽", 15).gain_exp(data.exp_for_level(16) - data.exp_for_level(15) + 1)
check("升级触发进化标记", evo == "藤蔓兽", str(evo))

mm = Mon("小岩蛇", 20)
old = mm.evolve_into("岩铠兽")
check("进化变更", old == "小岩蛇" and mm.species == "岩铠兽" and mm.max_hp > 30)
...
b = Battle(g, [Mon("啮齿鼠", 3)], callback=lambda r: None)
b.auto = True
r = b.drive_for_test([("menu", "fight"), ("move", 0)])
check("野外战斗胜利", r == "win", str(r))
check("敌人倒下", b.foe.hp == 0)
check("获得经验", g.party[0].exp > data.exp_for_level(20))
...
check("入队", any(m.species == "麻雀雏" for m in g.party))

# 换人(存活场景)+ 逃跑(高速必成功)
g.party = [Mon("芽叶兽", 8), Mon("电鼠", 12)]
b3 = Battle(g, [Mon("小岩蛇", 3)], callback=lambda r3: None)
b3.auto = True
r3 = b3.drive_for_test([("menu", "party"), ("party", 1)])
check("换人生效", b3.p_idx == 1)
check("换人后被攻击仍存活", b3.ally.hp > 0)
b3b = Battle(g, [Mon("小岩蛇", 2)], callback=lambda r3b: None)
b3b.auto = True
b3b.p_idx = 1
b3b._started = False
...
# 训练家战(道馆):打输触发 lose
g.party = [Mon("芽叶兽", 2)]
b4 = Battle(g, [Mon("岩铠兽", 40)], trainer_name="测试馆长", callback=lambda r4: None)
b4.auto = True
...
b5 = Battle(g, [Mon("小岩蛇", 10), Mon("小岩蛇", 10)], trainer_name="测试", callback=lambda r5: None)
b5.auto = True
...
while ow.state == "dialog" and steps < 20:
    ow.handle_event(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_z))
    steps += 1
check("获得初始精灵", len(g2.party) == 1 and g2.party[0].species == "芽叶兽")
...
g3 = Game()
g3.load()
check("读档队伍", len(g3.party) == 1 and g3.party[0].species == "芽叶兽")
...
ow4.state = "field"
g4.party = [Mon("火苗狐", 10)]
b6 = g4.push_battle([Mon("麻雀雏", 3)], callback=lambda r6: None)
b6.draw(surf)
b6.phase = "menu"
b6.draw(surf)
check("各场景绘制无异常", True)
